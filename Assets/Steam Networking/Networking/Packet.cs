...
public class Packet : IDisposable
{
    ...
    public void InsertFloat(float _value)
    {
        buffer.InsertRange(0, BitConverter.GetBytes(_value)); // Insert the int at the start of the buffer
    }
    ...
    public void Write(bool[] _value)
    {
        ...
        BitArray bits = new BitArray(_value);
        byte[] bytes = new byte[_value.Length / 8 + (_value.Length % 8 == 0 ? 0 : 1)];
        bits.CopyTo(bytes, 0);
    ...
    public ushort ReadUShort(bool _moveReadPos = true)
    {
        ...
        {
            // If there are unread bytes
            ushort _value = BitConverter.ToUInt16(readableBuffer, readPos); // Convert the bytes to an int
            if (_moveReadPos)
            {
                // If _moveReadPos is true
                readPos += 2; // Increase readPos by 2
    ...
    public bool[] ReadBoolArray(bool _moveReadPos = true)
    {
        if (buffer.Count > readPos)
        {
            ...
            byte[] bytes = ReadBytes(length / 8 + (length % 8 == 0 ? 0 : 1));
            ...
            return array; // Return the bool
        }
        else
        {
            throw new Exception("Could not read value of type 'bool array'!");
        }
    }
